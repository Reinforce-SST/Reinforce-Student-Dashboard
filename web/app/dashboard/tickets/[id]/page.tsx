...
export default function TicketPage() {
  ...
  return <div className={styles.page}>
    <Link href="/dashboard/tickets" className={styles.textLink}>Back to your tickets</Link>
    {error ? <div role="alert" className={styles.card}><p>{error}</p><button className={styles.button} onClick={() => { setState(null); setAttempt(value => value + 1); }}>Retry</button></div> : !data ? <p role="status">Loading the conversation…</p> : <>
      <div className={styles.intro}><p className={styles.eyebrow}><span className={styles.badge} data-status={data.ticket.status}>{STATUS_LABEL[data.ticket.status]}</span></p><h1 className={styles.title}>{data.ticket.title}</h1><p className={styles.muted}>{data.ticket.description}</p></div>
      <div className={styles.workspace}>
        <section className={styles.section}><div className={styles.sectionHead}><h2>Conversation</h2></div><p className={styles.accountHint}>Latest 300 messages. Reply in Discord to keep the conversation together.</p>{data.messages.length === 0 ? <p className={styles.empty}>No messages have been synced yet.</p> : data.messages.map(message => <article key={message.id} className={styles.message}><strong>{message.sender_name}</strong>{message.timestamp && <p className={styles.muted}><time dateTime={message.timestamp}>{new Date(message.timestamp).toLocaleString()}</time></p>}<p>{message.content}</p>{message.attachments.map((url, index) => safeUrl(url) ? <a key={`${url}:${index}`} href={safeUrl(url)!} target="_blank" rel="noreferrer">Attachment {index + 1}</a> : null)}</article>)}</section>
        <aside className={styles.card}><h2>Request details</h2><dl className={styles.facts}>{data.ticket.fields.map(field => <div key={field.label}><dt>{field.label}</dt><dd>{field.value}</dd></div>)}</dl>{data.ticket.close_reason && <p className={styles.muted}>Closed: {data.ticket.close_reason}</p>}{data.ticket.thread_url && safeUrl(data.ticket.thread_url) && <div className={styles.actions}><a href={safeUrl(data.ticket.thread_url)!} target="_blank" rel="noreferrer" className={styles.button}>Continue in Discord</a></div>}</aside>
      </div>
    </>}
  </div>;
}
